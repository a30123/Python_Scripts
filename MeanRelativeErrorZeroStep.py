...
output_folder_filename="C://Users//A30123.ITRI//Documents//Python Scripts//New_for_event_mining//Try_20150504_feature_mean_relative_error//mean_relative_error_zero_step.csv"
#########################################################################################################
#######################################   MAIN PROGRAM        ###########################################
#########################################################################################################
tstart = time.time()
...
for i in range(len(files_in_folder)):
    ...
    zero_step_occurrence=~(category_values_float==-1)    
    
    if (sum(zero_step_occurrence)!=0):
        relative_error_float=np.array(relative_error_values,dtype='float16')
        relative_error_to_consider=relative_error_float[zero_step_occurrence]
        
        mean_relative_error_float=np.mean(relative_error_to_consider)
        mean_relative_error_list.append(mean_relative_error_float)
    else:
        mean_relative_error_list.append(-0.5)
